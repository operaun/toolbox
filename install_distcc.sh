...
DISTCC_DIR=${HOME}/office/build/distcc
...
grep distccd_flags /etc/rc.conf || echo "distccd_flags=\"--allow 192.168.10.0/24 --listen ${MY_IP} --port 3632 --user distcc --log-file=/var/log/distccd.log --daemon -P /var/run/distccd.pid -N 20 -j 5\"" | sudo tee -a /etc/rc.conf > /dev/null
